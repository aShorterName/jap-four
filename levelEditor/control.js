...
id("levelLayer").addEventListener("mousedown", function (input) {
  if (!editDisabled) {
	...
	let z = player.selectCoordsM[2]
	let w = player.selectCoordsM[3]
    let xb = Math.floor((input.offsetX - camCenterx) / baseBlockSize);
    let yb = Math.floor((input.offsetY - camCentery) / baseBlockSize);
	let zb = player.selectCoords[2]
	let wb = player.selectCoords[3]
    let trueBlock = getBlock(x, y, z, w, false);
    let block = getBlock(x, y, z, w);
    let trueType = getBlockType(x, y, z, w, false);
    let type = block;
    ...
    if (control.e) {
      ...
      if (input.button === 0 && !locks.building) {
        ...
        if (hasProperty(player.selectedBlock[0])) {
          editBlock(x, y, z, w, [player.selectedBlock[0]], player.miniBlock);
        ...
        } else editBlock(x, y, z, w, player.selectedBlock[0], player.miniBlock);
        if (
          player.miniBlock &&
          arraysEqual(level[wb][zb][xb][yb][1], level[wb][zb][xb][yb][2]) &&
          arraysEqual(level[wb][zb][xb][yb][2], level[wb][zb][xb][yb][3]) &&
          arraysEqual(level[wb][zb][xb][yb][3], level[wb][zb][xb][yb][4])
        )
          level[wb][zb][xb][yb] = level[wb][zb][xb][yb][1];
        drawLevel();
        ...
        if (hasProperty(player.selectedBlock[1])) {
          editBlock(x, y, z, w, [player.selectedBlock[1]], player.miniBlock);
          for (let i in defaultProperty[player.selectedBlock[1]]) {
            editProp(
              x,
              y,
        ...
        } else editBlock(x, y, z, w, player.selectedBlock[1], player.miniBlock);
        if (
          player.miniBlock &&
          arraysEqual(level[wb][zb][xb][yb][1], level[wb][zb][xb][yb][2]) &&
          arraysEqual(level[wb][zb][xb][yb][2], level[wb][zb][xb][yb][3]) &&
...
id("levelLayer2").addEventListener("mousedown", function (input) {
  if (!editDisabled) {
    input.preventDefault();
    window.focus();
    let z = Math.floor(((input.offsetX - camCenterz) / baseBlockSize) * 2) / 2;
    let w = Math.floor(((input.offsetY - camCenterw) / baseBlockSize) * 2) / 2;
	let x = player.selectCoordsM[0]
	let y = player.selectCoordsM[1]
    let zb = Math.floor((input.offsetX - camCenterz) / baseBlockSize);
    let wb = Math.floor((input.offsetY - camCenterw) / baseBlockSize);
	let xb = player.selectCoords[0]
	let yb = player.selectCoords[1]
    let trueBlock = getBlock(x, y, z, w, false);
    let block = getBlock(x, y, z, w);
    let trueType = getBlockType(x, y, z, w, false);
    let type = block;
...
id("levelLayer").addEventListener("mousemove", function (input) {
  if (!editDisabled && !locks.building) {
    ...
    if (input.ctrlKey) {
      ...
      if (control.lmb) {
        if (player.miniBlock && trueType !== 73)
          level[bw][bz][xb][yb] = [73, trueBlock, trueBlock, trueBlock, trueBlock];
        if (player.selectedBlock[0] === 17) setSpawn(x, y, true);
        if (hasProperty(player.selectedBlock[0])) {
          editBlock(x, y, z, w, [player.selectedBlock[0]], player.miniBlock);
          for (let i in defaultProperty[player.selectedBlock[0]]) {
        ...
        if (
          player.miniBlock &&
          arraysEqual(level[bw][bz][xb][yb][1], level[bw][bz][xb][yb][2]) &&
          arraysEqual(level[bw][bz][xb][yb][2], level[bw][bz][xb][yb][3]) &&
          arraysEqual(level[bw][bz][xb][yb][3], level[bw][bz][xb][yb][4])
        )
          level[bw][bz][xb][yb] = level[bw][bz][xb][yb][1];
        drawLevel();
      } else if (control.rmb) {
        if (player.miniBlock && trueType !== 73)
          level[bw][bz][xb][yb] = [73, trueBlock, trueBlock, trueBlock, trueBlock];
...
id("levelLayer2").addEventListener("mousemove", function (input) {
  if (!editDisabled && !locks.building) {
    input.preventDefault();
    let z = Math.floor(((input.offsetX - camCenterz) / baseBlockSize) * 2) / 2;
    let w = Math.floor(((input.offsetY - camCenterw) / baseBlockSize) * 2) / 2;
    ...
    if (input.ctrlKey) {
      if (control.lmb && !locks.panning) {
        player.playerFocus = false;
        lvlzOffset += input.movementX;
        lvlwOffset += input.movementY;
        camz = lvlzOffset;
        camw = lvlwOffset;
        adjustScreen();
      }
...
document.addEventListener("mouseup", function (input) {
  if (input.button === 0) {
    control.lmb = false;
